...
async def my_results_handler(request: web.Request):
    ...
    aplus_rows = await db.get_user_aplus_test_results(user["id"])
            ...
            "aplus_results": [
                {
                    ...
                    "code": row["code"],
                    "score": row["score"],
                    "total_questions": (row["question_count"] or 1) * 2,
                    "submitted_at": row["submitted_at"],
                }
...
def _validate_schedule_body(body):
    ...
    time_of_day = body.get("time_of_day") or ""
    if not isinstance(day_of_week, int) or not (0 <= day_of_week <= 6):
        return None, None, web.json_response({"error": "invalid_day"}, status=400)
    if not re.fullmatch(r"[0-2]\d:[0-5]\d", time_of_day):
        return None, None, web.json_response({"error": "invalid_time"}, status=400)
    return day_of_week, time_of_day, None


async def save_report_schedule_handler(request: web.Request):
    """Yangi hisobot vaqti QO'SHADI (mavjudlarga ustiga qo'shiladi, ularni
    almashtirmaydi - masalan bir nechta kun/vaqt bo'lishi mumkin)."""
    try:
        body = await request.json()
    except json.JSONDecodeError:
        return web.json_response({"error": "bad_request"}, status=400)
    ...
    day_of_week, time_of_day, error = _validate_schedule_body(body)
    if error:
        return error
...
async def update_report_schedule_handler(request: web.Request):
    ...
    enabled = bool(body.get("enabled", True))
    await db.update_report_schedule(schedule_id, day_of_week, time_of_day, enabled)
