...
// createMathKeyboard(mountEl) -> { toggle(fieldEl, toggleBtnEl), hide(), isOpen }
//
...
function createMathKeyboard(mountEl) {
  ...
  function focusInput(fieldEl, toggleBtn) {
    // Klaviatura ochiq bo'lsa, foydalanuvchi boshqa maydonga to'g'ridan-to'g'ri
    // (⌨ tugmasini bosmasdan) o'tganda ham, klaviatura O'SHA yangi maydonga
    // yozishni davom ettirishi uchun - hech qachon yashirmaydi, faqat
    // "faol maydon"ni yangilaydi.
    if (!root.hidden && activeField !== fieldEl) {
      show(fieldEl, toggleBtn);
    }
  }
